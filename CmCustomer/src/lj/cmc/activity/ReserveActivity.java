...
import lj.cmc.string_analysis.DataOfShopsAnalyzeHelper;
		...
		public void doSome() {
			...
			if(analyzeHelper.recode==AppConstant.ReCodeFinal.OK.code){//成功,加载数据				
				...
				for(int i=0;i<size;i++){
					...
					hashTemp.put("lisli_shopHoursTv", new SpannableString("营业时间:"+sdfTime.format(shopInfo.shopHoursBeginTime)+"-"+sdfTime.format(shopInfo.shopHoursEndTime)));
					list.add(hashTemp);
				}		
				//更新适配器
