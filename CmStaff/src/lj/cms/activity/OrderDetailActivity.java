			...
			if(analyzeHelper.recode==AppConstant.ReCodeFinal.OK.code){//成功,加载数据				
				...
				if(dataList==null){
					...
					if(analyzeHelper.orderInfo!=null){
						...
						odAy_dateTv.setText("用餐日期:"+analyzeHelper.orderInfo.date);//用餐日期
						odAy_timeTv.setText("用餐时间:"+analyzeHelper.orderInfo.time);//用餐时间
...
//	                                    htmlTag += "<a href='" + createLink(controller: "staff", action: "doDish", params: [orderId: orderId]) + "'>点菜</a>&nbsp;&nbsp;";
				...
				if(null==iatclAdapter){
					iatclAdapter=new ITBCListAdapter(OrderDetailActivity.this, odAy_dishListLv, list,R.layout.lvitem_dish_list_item, 
							new String[]{"lidli_dishCzQxBt","lidli_dishCzBt","lidli_foodNameTv","lidli_priceTv","lidli_countTv","lidli_statusTv","lidli_validTv","lidli_cancelReasonTv"},
							new int[]{R.id.lidli_dishCzQxBt,R.id.lidli_dishCzBt,R.id.lidli_foodNameTv,R.id.lidli_priceTv,R.id.lidli_countTv,R.id.lidli_statusTv,
