 ...
 */
sy.refreshTab=function(cfg){

    var tabs=cfg.tabs,refresh_tab;
    if(cfg.tabTitle)
     refresh_tab= tabs.tabs('getTab',cfg.tabTitle)
    else if(cfg.tabIndex)
     refresh_tab=tabs.tabs('getTab',cfg.tabIndex)
    else
    ...
    if(refresh_tab && refresh_tab.find('iframe').length > 0){
        ...
        tabs.tabs('update',{
              tab: refresh_tab,
              options: {
                      content : '<iframe src="'+cfg.url+'" frameborder="0" style="border:0;width:100%;height:99.4%;"></iframe>'
              }
        });
    }
}

sy.addTab=function(tabs,title, href,icon){
    var tt = tabs;
 ...
 * 数组1减去数组2
 */
sy.arrayMinus=function(array1,array2){
    if(array2.length==0){return array1}
    var diff=[];
    var str=array2.join("&quot;&quot;");
    for(var e in array1){
        if(str.indexOf(array1[e])==-1){
            diff.push(array1[e]);
        }
    }
    return diff;
...
sy.rowMinus=function(row1,row2){
    if(row2.length==0) return row1
    var diff=[]
    var str=''
    for(var e in row2){
        str+=row2[e].id+";"
