...
<script type="text/javascript" charset="utf-8">
    ...
    $(function() {
        datagrid = $('#datagrid').datagrid({
            url : "${resource()}/CuisineInfo/datagrid",
            pagination : true,
            pagePosition : 'bottom',
            pageSize : 10,
    ...
    function addEasyui() {
        var p = parent.sy.dialog({
            title : '新增 菜系',
            href : "${resource()}/CuisineInfo/edit",
            width : 600,
            height : 280,
            buttons : [ {
                text : '新增',
                handler : function() {
                    var f = p.find('form');
                    f.form({
                        url : "${resource()}/CuisineInfo/addSaveHandle",
                        success : function(d) {
            ...
            var p = parent.sy.dialog({
                ...
                buttons : [ {
                    ...
                    handler : function() {
                        var f = p.find('form');
                        f.form('submit',{
                            url : "${resource()}/CuisineInfo/editUpdHandle",
                            success : function(d) {
                                var json = $.parseJSON(d);
                                if (json.success) {
                                    datagrid.datagrid('reload');
