...
<head>
  ...
  <title>外卖包装方式管理</title>

<script type="text/javascript" charset="utf-8">
    var datagrid;
    $(function() {
        datagrid = $('#datagrid').datagrid({
            url : "${resource()}/packInfo/datagrid",
            pagination : true,
            pagePosition : 'bottom',
            pageSize : 10,
        ...
        var p = parent.sy.dialog({
            ...
            buttons : [ {
                ...
                handler : function() {
                    var f = p.find('form');
                    f.form({
                        url : "${resource()}/packInfo/addSaveHandle",
                        success : function(d) {
            ...
            var p = parent.sy.dialog({
                ...
                buttons : [ {
                    ...
                    handler : function() {
                        var f = p.find('form');
                        f.form('submit',{
                            url : "${resource()}/packInfo/editUpdHandle",
                            success : function(d) {
                                var json = $.parseJSON(d);
                                if (json.success) {
                                    datagrid.datagrid('reload');
